...
Small english model from spacy.
It starts with ! as it will run the line in the terminal.
nlp is our text processing object to use later in the code.
...
!get command line utility that takes a file from the url that you give and save it into the input directory of the container.
...
Collection of political speeches from the British parliament.

f string embeds a variable into a string. Automatically create names of files.
...
Each row in the dataframe is a political speech.
...
British=true because Canadian english is also in the package. The data subset we are working with contains `{python} len(bh1620)` speeches.

## Data Processing

Let's confirm that we have the correct years.
Data is available since the 1970s.
...
Dropping missing data leaves us with `{python} len(bh1620)` speeches. Over 300000 speeches.
...
parties_keep = [
    ...
    'Plaid Cymru',
    'Green Party', 'Social Democratic & Labour Party'
]
...
We take a 10% sample: percent computed over speeches for each party.
...
We want to assess if very short speeches are still counted as such.
...
With apply go through strings and then group them by party, count the number of words spoken by each party and compare them.
...
fig, ax = plt.subplots(3, 4, sharex=True, sharey=True, figsize=(20, 6))
...
party_subplot(parties.get_group('Social Democratic & Labour Party'), 'Social Democratic & Labour Party', ax[2, 0])
...
Text processing is a crucial step before applying the kinds of text analysis techniques we will apply here.^[This is not the case for the transformer-based approaches we will consider later.] Common steps include removing stopwords, converting to lowercase, lemmatization, and detecting bigrams (e.g., climate change, european union) --> train the model to check for more frequent pairings and treat them as one word (different meaning from words used alone). We will use the preprocess() function from the course package to perform these operations.
...
It is a quantitative representation and we use a function from sklearn.
...
Compare how often some words are used by each party.
...
With this dataframe, we can compare the proportions of specific tokens across each party. Unsurprisingly, the SNP uses 'scotland' most often.
...
This is all counting and comparing things at a manifest level. Nothing is done at the latent level.
...
tfidf (term frequency inverse document frequency) instead of a count factorizer. We also normalize the matrix, as is typically done.

Normalization is a step that is typical of LSAs --> it is needed to obtain optimized content for longer speeches.
...
We can inspect the singular values to understand the importance of each latent dimension (weights if each word).
...
The matrix decomposition takes every word in the dataset under different topics. We can then compare how each word goes under the different topics.
...
Loadings for each word under each latent topic.
Next step: top weighted words for each topic; search for other words you are interested in.
LSA is a deterministic method. It will yield the same results every time you run it.
...
They use a generative language model, that takes speeches from people and completes them.

LDAs model a data generating process using a probabilistic model.

LDAs assume that there are latent topics that affect the way that words appear in documents in relation to one another.
...
We only give LDA a bag of words with counts --> count-based representation without any other information on the words.
dock2bow== dock to bag of words. It is another way of having stopwords out.
In topic modelling, there is no way of understanding how many topics are in a dataset --> use num_topics.
...
Next, we'll fit an LDA model to a sample of our data (1000 data). Small sample to get this loading in a smaller amount of time.
...
Freedom is not in the speeches.

```{python}
ldamod_s.get_term_topics('freedom')
```

Select topics to look at and have a probabilistic assignment of words to those topics.
...
Look at topics with scotland in it.

```{python}
ldamod_s.show_topic(6, topn=10)
```

```{python}
ldamod_s.show_topic(33, topn=10)
```

```{python}
ldamod_s.show_topic(69, topn=10)
```

```{python}
ldamod_s.show_topic(91, topn=10)
```

These objects are all tuples. The first item is an id for a topic.
You can search topics and do searches of the topics to find the most strongly associated words.
In this generation of topic models, typically there is a combination of quantitative modelling and more qualitative variables.
...
This kind of model also performs contextual embeddings.
Embeddings are basically the elements of matrixes.
Word embeddings condense the representation of the world.
You can also do mathematical operations on vectors (e.g., subtract man from king and get queen).
Still, they only have one meaning.
Contextual embeddings look at words around the word of interest and count for different embeddings.
This takes an enormous amount of data and of energy to run.
OS models examples: BERT, LAMA.
These can be trained and you can interact with them, through transformer-based models.
They are not specialised models, but have general knowledge.
They then need to be finetuned into tasks that we need.
BERTopic uses BERT for topic analysis.
...
Data need not be preprocessed in this case. Use 'speech' and not only a sample.
The output is twofold: topics and probs.
...
Using BERTopic functions.
BERT uses the best possible representation for that data:
...
keyword --> uses representations through keywords.
-1 ist the only topic number that has meaning, and it indicates junk topics.
...
Finally, we can get information about how documents are classified into topics through a BERTopic function.

```{python}
topic_model.get_document_info(docs)
```

Information of most probable information in some topics.
...
Full information on BERTopic are available in the website.
